...
function MarkEntryPage() {
  ...
  const fetchMarkCategories = async () => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/mark-categories-by-type/${courseTypeId}`)
  ...
  const loadExistingMarks = async () => {
    try {
      const response = await fetch(`${API_BASE_URL}/course/${selectedCourse.course_id}/student-marks`)
