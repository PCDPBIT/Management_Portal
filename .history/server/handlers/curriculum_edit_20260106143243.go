...
func UpdateCurriculum(w http.ResponseWriter, r *http.Request) {
	...
	err = db.DB.QueryRow("SELECT name, academic_year, max_credits FROM curriculum WHERE id = ?", curriculumID).Scan(&oldName, &oldAcademicYear, &oldMaxCredits)
	if err != nil {
	...
	// Update curriculum
	query := "UPDATE curriculum SET name = ?, academic_year = ?, max_credits = ? WHERE id = ?"
	_, err = db.DB.Exec(query, updateData.Name, updateData.AcademicYear, updateData.MaxCredits, curriculumID)
	if err != nil {
