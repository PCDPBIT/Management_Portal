...
func GetCourseExperiments(w http.ResponseWriter, r *http.Request) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&exp.ID, &exp.CourseID, &exp.ExperimentNumber, &exp.ExperimentName); err != nil {
			continue
		}
...
func CreateExperiment(w http.ResponseWriter, r *http.Request) {
	...
	result, err := db.DB.Exec(`
		INSERT INTO course_experiments (course_id, experiment_number, experiment_name)
		VALUES (?, ?, ?)`, courseID, req.ExperimentNumber, req.ExperimentName)
	if err != nil {
...
func UpdateExperiment(w http.ResponseWriter, r *http.Request) {
	...
	_, err = db.DB.Exec(`
		...
		SET experiment_number = ?, experiment_name = ?
		WHERE id = ?`, req.ExperimentNumber, req.ExperimentName, expID)
	if err != nil {
