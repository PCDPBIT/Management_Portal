...
// UpdateCourseTheoryHours calculates and updates the theory_hours field based on module/unit hours sum
func UpdateCourseTheoryHours(courseID int) error {
	// Get curriculum template to determine if we should use modules or experiments
	template := getCurriculumTemplateForCourse(courseID)
	
	var totalHours int
	
	if template == "2022" {
		// For 2022: sum hours from experiments
		err := db.DB.QueryRow(`
			SELECT COALESCE(SUM(hours), 0)
			FROM course_experiments
			WHERE course_id = ?`, courseID).Scan(&totalHours)
		if err != nil {
			log.Println("Error calculating experiment hours:", err)
			return err
		}
	} else {
		// For 2026: sum hours from module titles
		err := db.DB.QueryRow(`
			SELECT COALESCE(SUM(st.hours), 0)
			FROM syllabus_titles st
			INNER JOIN syllabus_models sm ON st.model_id = sm.id
			WHERE sm.course_id = ?`, courseID).Scan(&totalHours)
		if err != nil {
			log.Println("Error calculating module hours:", err)
			return err
		}
	}
	
	// Update the course theory_hours
	_, err := db.DB.Exec(`
		UPDATE courses
		SET theory_hours = ?
		WHERE course_id = ?`, totalHours, courseID)
	
	if err != nil {
		log.Println("Error updating course theory_hours:", err)
		return err
	}
...
func CreateTitle(w http.ResponseWriter, r *http.Request) {
	...
	// Get course_id from model and update theory hours
	var courseID int
	db.DB.QueryRow("SELECT course_id FROM syllabus_models WHERE id = ?", modelID).Scan(&courseID)
	updateCourseTheoryHours(courseID)
...
func UpdateTitle(w http.ResponseWriter, r *http.Request) {
	...
	db.DB.QueryRow(`
		...
		FROM syllabus_titles st
		INNER JOIN syllabus_models sm ON st.model_id = sm.id
		WHERE st.id = ?`, titleID).Scan(&courseID)
	updateCourseTheoryHours(courseID)
...
func DeleteTitle(w http.ResponseWriter, r *http.Request) {
	...
	// Update theory hours after deletion
	updateCourseTheoryHours(courseID)
