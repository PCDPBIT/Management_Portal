...
function ClauseSidebar({ structure, selectedClause, onClauseSelect, onSectionAdd, onClauseAdd, canEdit }) {
  ...
  const [newClause, setNewClause] = useState({ clause_no: '', title: '', content: '' })
  ...
  const handleClauseSubmit = (e, sectionId) => {
    ...
    setNewClause({ clause_no: '', title: '', content: '<p>Clause content...</p>' })
  ...
  return (
    <div className="card-custom p-4 h-[calc(100vh-250px)] overflow-y-auto">
      ...
      {/* Add Section Form */}
      {showSectionForm && (
        <form onSubmit={handleSectionSubmit} className="mb-4 p-3 bg-blue-50 rounded-lg space-y-2">
          <input
            type="number"
            placeholder="Section No. (e.g., 1)"
            value={newSection.section_no}
            onChange={(e) => setNewSection({ ...newSection, section_no: parseInt(e.target.value) })}
            required
          ...
          <div className="flex space-x-2">
            <button type="submit" className="flex-1 px-2 py-1 text-xs bg-blue-600 text-white rounded hover:bg-blue-700">
              Add
            </button>
            <button
              type="button"
        ...
        {structure && structure.length > 0 ? (
          structure.map((section) => (
            <div key={section.id} className="border border-gray-200 rounded-lg overflow-hidden">
              {/* Section Header */}
              <div className="flex items-center justify-between bg-gray-50 px-3 py-2 cursor-pointer hover:bg-gray-100"
                   onClick={() => toggleSection(section.id)}>
                <div className="flex items-center space-x-2 flex-1">
                  <svg 
                    className={`w-4 h-4 text-gray-500 transition-transform ${expandedSections.has(section.id) ? 'transform rotate-90' : ''}`}
                    fill="none" 
                    stroke="currentColor" 
                    viewBox="0 0 24 24"
                  >
                ...
                {canEdit && (
                  <button
                    onClick={(e) => {
                      e.stopPropagation()
                      setShowClauseForm(showClauseForm === section.id ? null : section.id)
                    }}
              ...
              {showClauseForm === section.id && (
                ...
                >
                  <input
                    ...
                    onChange={(e) => setNewClause({ ...newClause, clause_no: e.target.value })}
                    required
                  ...
                  />
                  <div className="flex space-x-2">
                    <button type="submit" className="flex-1 px-2 py-1 text-xs bg-green-600 text-white rounded hover:bg-green-700">
                      Add Clause
                    </button>
                    <button
                      type="button"
              ...
              {/* Clauses List */}
              {expandedSections.has(section.id) && section.clauses && section.clauses.length > 0 && (
                  ...
                  {section.clauses.map((clause) => (
                    <div
                      ...
                      className={`px-4 py-2 cursor-pointer border-t border-gray-200 hover:bg-blue-50 transition-colors ${
                        selectedClause?.id === clause.id ? 'bg-blue-100 border-l-4 border-l-blue-600' : ''
                      }`}
                    >
                      <div className="flex items-start space-x-2">
                        <span className="text-xs font-mono text-gray-500 mt-0.5">{clause.clause_no}</span>
                        <span className="text-sm text-gray-700 flex-1 line-clamp-2">{clause.title}</span>
                      </div>
                    </div>
                  ))}
                </div>
          ...
          <div className="text-center py-8 text-gray-500 text-sm">
            <p>No sections yet.</p>
            {canEdit && <p className="text-xs mt-1">Click + to add a section.</p>}
          </div>
        )}
      </div>
    </div>
