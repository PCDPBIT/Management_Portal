...
function SyllabusPage() {
  ...
  const handleUpdateExperiment = async (expId, currentName, currentNumber) => {
    const newName = prompt('Enter new experiment name:', currentName)
    if (!newName || newName === currentName) return
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/experiments/${expId}`, {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          experiment_number: currentNumber,
    ...
    >
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="card-custom p-6">
          ...
          {activeTab === 'experiments' && curriculumTemplate === '2022' && (
            <div>
              <p className="text-gray-600 mb-4">Experiments for this course (2022 Template)</p>
              <button className="btn-primary-custom">+ Add Experiment</button>
            </div>
          )}
