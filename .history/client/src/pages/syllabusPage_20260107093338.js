...
function SyllabusPage() {
  ...
  const handleAddExperiment = async () => {
    ...
    const expName = prompt(`Enter name for Experiment ${expNumber}:`)
    if (!expName) return
  ...
  const handleUpdateExperiment = async (expId, newName, newTopics = null) => {
    const exp = experiments.find(e => e.id === expId)
    if (!exp) return

    // If called with just ID and name (from name input field), prompt for new name
    if (typeof newName === 'string' && newName !== exp.experiment_name && !newTopics) {
      // Direct update from input field
      const experimentName = newName
      try {
          ...
          <div key={experiment.id} className="border border-gray-200 rounded-lg overflow-hidden bg-white shadow-sm">
            ...
            <div className="bg-gray-50 px-4 py-3 flex justify-between items-center border-b border-gray-200">
              <div className="flex items-center gap-3 flex-1">
                <span className="font-semibold text-gray-700">Experiment {experiment.experiment_number}</span>
                <input
                  type="text"
                  value={experiment.experiment_name}
                  onChange={(e) => handleUpdateExperiment(experiment.id, e.target.value)}
                  className="input-custom flex-1"
