...
function SemesterDetailPage() {
  ...
  const fetchCourses = async () => {
    try {
      setLoading(true)
      const response = await fetch(`${API_BASE_URL}/api/regulation/${id}/semester/${semId}/courses`)
      if (!response.ok) {
  ...
  const handleAddCourse = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/regulation/${id}/semester/${semId}/course`, {
        method: 'POST',
        headers: {
  ...
  const handleUpdateCourse = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/course/${editingCourse.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleRemoveCourse = async (courseId) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/regulation/${id}/semester/${semId}/course/${courseId}`, {
        method: 'DELETE',
      })
            ...
            <form onSubmit={handleAddCourse} className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input
                  ...
                  title="This is auto-calculated from modules/units hours"
                />
              </div>
              ...
              <form onSubmit={handleUpdateCourse} className="p-8 space-y-5">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  ...
                  <div>
                    ...
                    <input
                      type="number"
                      value={(parseInt(editCourseData.theory_hours) || 0) + (curriculumTemplate !== '2022' ? (parseInt(editCourseData.activity_hours) || 0) : 0) + (parseInt(editCourseData.lecture_hours) || 0) + (parseInt(editCourseData.tutorial_hours) || 0) + (parseInt(editCourseData.practical_hours) || 0)}
                      readOnly
                      className="input-custom bg-gray-100 cursor-not-allowed"
                    />
                  </div>
